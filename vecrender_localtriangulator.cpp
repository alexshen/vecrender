...
void LocalTriangulatorTriangle::setIndices(
...
void LocalTriangulator::setPoints(const glm::vec2* points, int numPoints)
{
    ...
    for (int i = 0; i < numPoints; ++i) {
        DT::Vertex_handle vh = dt.insert(DT::Point(points[i].x, points[i].y));
        vh->info().index = i;
    }
    for (DT::Face_handle fh : dt.finite_face_handles()) {
        addTriangle(fh->vertex(0)->info().index, fh->vertex(1)->info().index,
            fh->vertex(2)->info().index);
    }
}

void LocalTriangulator::addTriangle(
    std::uint8_t v0, std::uint8_t v1, std::uint8_t v2)
{
